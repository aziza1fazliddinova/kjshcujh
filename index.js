...
// a.arr = arr.filter(task =>task.completed ===true);
// a.count = a.arr.length;
// b.arr = arr.filter(task => task.completed ===false);
...
let other = []
users.forEach(item=>{
    if(item.email.includes('org')&&item.email.includes('net')&& item.email.includes('info')){
        org&&net&&info.push(item.email)
   }else{
